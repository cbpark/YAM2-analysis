...
int main(int argc, char *argv[]) {
    if (argc != 3) {
        std::cerr << "Usage: m2cc_mw_auglag_nmsimplex.exe input output\n"
                  << "  - input: Input file in "
    ...
    for (; !ev.empty(); ev = lhef::parseEvent(&infile), ++num_eve) {
        ...
        const auto m2sol = yam2::m2CCAugLagNMSimplex(input.value(), TOL);
        ...
        outfile << std::setw(12) << std::setprecision(7) << m2sol_val.m2()
                << std::setw(12) << std::setprecision(7) << mw.value()[0]
                << std::setw(12) << std::setprecision(7) << mw.value()[1]
